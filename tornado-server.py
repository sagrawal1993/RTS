...
class MainHandler(tornado.web.RequestHandler): #Class that renders login page
	...
	def get(self):
		...
		self.write(detail_template.render({"subscribers":len(subscribers)}))
...
def send_user_alert():
    while True:
        try:
            temp_conn = yield r.connect('localhost',28015,'test')
            feed = yield r.table("mytable").changes().run(temp_conn)
            print feed
...
if __name__ == "__main__": 
    ...
    tornado_app = tornado.web.Application([('/', MainHandler), #For Landing Page 
